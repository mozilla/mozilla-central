 ...
 *
 * $Id: ocsp.c,v 1.37 2007-03-23 05:35:15 kaie%kuix.de Exp $
 */
...
ocsp_CacheKeyHashFunction(const void *key)
{
    ...
    walk = cid->issuerNameHash.data;
    for (i=0; i < cid->issuerNameHash.len; ++i, ++walk) {
        hash += *(unsigned char*)walk;
    }
    walk = cid->issuerKeyHash.data;
    for (i=0; i < cid->issuerKeyHash.len; ++i, ++walk) {
        hash += *(unsigned char*)walk;
    }
    walk = cid->serialNumber.data;
                                       ...
                                       OCSPCacheItem **pCacheItem)
{
    ...
    *pCacheItem = NULL;
    ...
    OCSPCacheItem *item = (OCSPCacheItem *)
                          PORT_ArenaZAlloc(certID->poolp, 
