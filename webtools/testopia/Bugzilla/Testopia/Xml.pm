...
sub parse() {
    ...
    foreach my $twig_testplan ($root->children('testplan')) {
        my $author_id = login_to_id(trim($twig_testplan->att('author')));
        if ( ! $author_id ) {
            $self->error("Cannot find author '" . $twig_testplan->att('author') . "' in test plan '" . $twig_testplan->field('name') . "'.");
        }
        
        my $product = new Bugzilla::Product({name => $twig_testplan->field('product')});
        if ( ! defined($product) ) {
        ...
        foreach my $twig_attachments (@attachments) {
            my $submitter_id = login_to_id(trim($twig_attachments->field('submitter')));
            if ( ! $submitter_id ) {
                $self->error("Cannot find submitter '" . $twig_attachments->field('submitter') . "' in test plan '" . $twig_testplan->field('name') . "' attachment '" . $twig_attachments->field('description') . "'.");
            }
            my $attachment = {
    ...
    my $testcase = Bugzilla::Testopia::TestCase->new({});
    my %priority_ids;
    ...
    foreach my $twig_testcase ($root->children('testcase')) {
        ...
        my $author_id = login_to_id(trim($twig_testcase->att('author')));
        if ( ! $author_id ) {
            $self->error("Cannot find author '" . $twig_testcase->att('author') . "' in test case '" . $summary . "'.");
        }
        my $tester_id = login_to_id(entity_replace_xml($twig_testcase->field('defaulttester'),STRIP_BOTH));
        if ( ! $tester_id ) {
            $self->error("Cannot find default tester '" . $twig_testcase->field('defaulttester') . "' in test case '" . $summary . "'.");
        }
        ...
        foreach my $twig_attachments (@attachments) {
            my $submitter_id = login_to_id(trim($twig_attachments->field('submitter')));
            if ( ! $submitter_id ) {
                $self->error("Cannot find submitter '" . $twig_attachments->field('submitter') . "' in test case '" . $twig_testcase->field('summary') . "' attachment '" . $twig_attachments->field('description') . "'.");
            }
            my $attachment = {
                'description'  => entity_replace_xml($twig_attachments->field('description'),STRIP_BOTH),
                'filename'     => entity_replace_xml($twig_attachments->field('filename'),STRIP_BOTH),
    ...
    if ( ! defined $self->parse_error ) {
        ...
        foreach my $category ( @{$self->categories} ) {
            ...
            if ( ! check_case_category($category->{'name'}, $category->{'product_id'}) ) {
                $category = Bugzilla::Testopia::Category->create($category);
        ...
        foreach my $testplan ( @{$self->testplans} ){
            $testplan = Bugzilla::Testopia::TestPlan->create($testplan);
            $testplan->add_tag($_) foreach ( @{$self->tags} );
...
Test Cases, or Categories and stores them in Testopia if no errors
are detected.
...
=item C<error($string)>
 ...
 Description: Flags if an error has occured during parsing. If so, data will not be written
              to the database until all errors have been resolved.
         
 Params:      String - the error message to display
 
 Returns:     Nothing.
 
=item C<parse($xml, $filename)>
 
 Description: Parses the provided xml and pulls out the plan structure along with associated
              categories and test cases. It then stores this in the database if no errors are
              found.
 
 Params:      xml - string containing the xml that was read in from tr_importxml.pl
              filename - If a filename is provided instead, it will open that file and parse the xml.
...
Testopia::(TestPlan, TestCase, TestRun, Category, Build, Environment, XmlTestCase, XmlReferences)
