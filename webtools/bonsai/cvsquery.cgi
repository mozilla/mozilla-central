...
# Rights Reserved.
#
# Contributor(s): Ben Hsieh <ben.hsieh@gmail.com> (adds graph output button)
...
if ($show_graph) {
    ...
    my $headerinfo = ''; # headerinfo will be written enclosed in the <head> tag generated by PutsHeader

    $headerinfo .= '<link rel="stylesheet" type="text/css" href="bargraph.css" />';

    # this part sets up the variables checkinsbargraph.js expects
    ...
    while ($k < $max) {
        ...
        if ($k != $max) {  #commas after all but last
            $headerinfo .= ",";
        }
    }
    $headerinfo .= "]";

    # copied from print_ci, assumes all files from same cvsroot.
    $headerinfo .= "\n var fileURLtemplate = \"cvsview2.cgi?subdir=" .
        "&files=\&command=DIRECTORY&branch=$::query_branch" .
        "&root=" . $result->[0]->[$::CI_REPOSITORY] . "\" \n" ;

    # copied from cvsview2.cgi, do_directory.  same assumption
    $headerinfo .= "var dirURLtemplate = \"rview.cgi?dir=\&cvsroot="
            . $result->[0]->[$::CI_REPOSITORY] ."\" \n";

    $headerinfo .= "var dir = \"" . $::FORM{'dir'} ."\"\n";

    $headerinfo .= "</script><script type='text/javascript' src='checkinsbargraph.js' />";

    PutsHeader($t, "CVS Checkins", "$menu", $headerinfo, "handleLoad()");
    print "<div id='output'></div></html>";
    exit;
}

elsif (!$show_raw && !$show_xml) {
